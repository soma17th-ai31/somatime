...
- 형식: {"busy_blocks": [{"start": string, "end": string}], "summary": string, "recognized_phrases": [string]}
...
- **요일 범위는 사용자가 명시한 요일에만 적용합니다.** 예: "월요일 18시 이후 가능" 이면 월요일에만 18시 이전을 busy 로 간주, **화/수/목/금/토/일은 그대로 둡니다 (busy 추가하지 마세요)**. 사용자가 명시하지 않은 요일에 임의로 제약을 확장하지 마세요.
- "월~금 9-18 가능", "평일 9-18 가능", "주말 없음" 처럼 요일 그룹을 명시하면 그 그룹에만 적용합니다.
- "월~금 9-18 가능" 처럼 요일 표현은 회의 기간 내의 실제 날짜로 매핑합니다 (meeting.dates 참고).
- **★ 요일 → 날짜 매핑은 반드시 ISO 달력 기준으로 계산하세요.** 한국 달력에서 월=0, 화=1, 수=2, 목=3, 금=4, 토=5, 일=6. 예: 2026-05-16 의 weekday 는 5 (토요일), 2026-05-18 의 weekday 는 0 (월요일). 사용자가 "월요일" 이라고 하면 weekday=0 인 meeting.dates 의 날짜만 매핑, "토요일" 이라고 하면 weekday=5 인 날짜만. 절대 임의 매핑 금지.
- summary 에 요일 단어 (월요일/화요일/.../토요일/일요일) 를 직접 쓰지 마세요. 대신 날짜만 "M/D" 또는 "YYYY-MM-DD" 로 표기. 예: "5/18 전체를 불가능으로 처리했습니다." (요일 단어 사용 시 잘못된 매핑으로 사실 오류가 발생할 수 있음).
...
recognized_phrases (FE의 chip 표시용) 작성 가이드:
- 사용자 자연어 입력에서 식별한 시간 조건을 **짧은 한국어 표현 3-6개**로 추출합니다.
- 각 항목은 12자 이내 권장. 요일/시간/가능여부가 한눈에 보이도록 작성합니다.
- 형식 예: "월 9-12시 불가", "화 ~18시 가능", "수 종일 가능", "목 12-18시", "주말 없음".
- 시각은 24시간제 숫자만, 0 패딩 없이 (9, 14, 18). 분이 30이 아니면 시까지만 표기 (9-12시).
- 요일은 한 글자(월/화/수/목/금/토/일) 또는 "주말", "평일", "내일" 등.
- 가능/불가는 명시적으로: "가능", "불가", "있음", "없음". 애매하면 시간 자체만 적습니다.
- 사용자가 명시하지 않은 조건은 chip에 만들지 않습니다 (보집합으로 추론된 busy_blocks 는 chip 대상이 아님).
- busy_blocks 가 빈 배열일 때 ("전부 가능" 등) recognized_phrases 도 "전부 가능" 같은 한 항목만 둘 수 있습니다.
- 입력에서 시간 조건을 전혀 추출할 수 없으면 빈 배열 [] 를 반환합니다.
...
출력:
{
  ...
  "recognized_phrases": ["5/12 14-17시 가능"]
}
